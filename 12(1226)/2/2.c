...
void Dijkstra_matrix(struct cell *adjacency_list[N], int start);
...
int main(int argc, char **aegv) {
    ...
    Dijkstra_matrix(adjacency_list, 0);
...
void Dijkstra_matrix(struct cell *adjacency_list[N], int start) {
    ...
    // Add the missing semicolon in the previous for loop
    for (i = 0; i < N; i++) {
        // Find the vertex with the minimum distance not in the set S
        v_k = -1;
        for (j = 0; j < N; j++) {
        ...
        // Include v_k in the set S
        S[v_k] = 1;

        // Update the distances
        current = adjacency_list[v_k];
        while (current != NULL) {
            if (!S[current->end_vertex] &&
